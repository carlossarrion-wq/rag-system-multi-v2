...
class MultiAppConversationalAgent(AdvancedConversationalAgent):
    ...
    def _generate_response_with_confidence_multi_app(
        ...
        # Get enhanced context for Haiku 3 caching optimization (CHUNK-BASED)
        enhanced_context_data = None
        if self.context_enhancer:
            try:
                # NEW: Use chunk-based context enhancement instead of sending ALL documents
                enhanced_context_data = self.context_enhancer.get_enhanced_context_from_chunks(query, search_results)
                print(f"📋 Enhanced context: {enhanced_context_data['context_length']} chars (Cache optimized: {enhanced_context_data['cache_optimized']}, Source: {enhanced_context_data.get('context_source', 'unknown')})")
                print(f"🎯 Context scope: {enhanced_context_data.get('document_count', 0)} documents from {enhanced_context_data.get('chunk_count', 0)} retrieved chunks")
            except Exception as e:
                print(f"⚠️  Warning: Could not get chunk-based enhanced context: {e}")
                # Fallback to old method if needed (but log the fallback)
                try:
                    enhanced_context_data = self.context_enhancer.get_enhanced_context(query)
                    print(f"📋 Fallback context: {enhanced_context_data['context_length']} chars (ALL documents - not optimized)")
                except Exception as fallback_error:
                    print(f"⚠️  Warning: Fallback context also failed: {fallback_error}")
...
IMPORTANTE: Utiliza este inventario documental para proporcionar respuestas más completas y precisas. Referencia documentos específicos cuando sea relevante. Esta información adicional debe integrarse naturalmente en tu respuesta JSON.
    ...
    def _generate_streaming_response_multi_app(
        ...
        # Get enhanced context for Haiku 3 caching optimization (CHUNK-BASED for streaming too)
        enhanced_context_data = None
        if self.context_enhancer:
            try:
                # NEW: Use chunk-based context enhancement for streaming as well
                enhanced_context_data = self.context_enhancer.get_enhanced_context_from_chunks(query, search_results)
                print(f"📋 Enhanced context (streaming): {enhanced_context_data['context_length']} chars (Cache optimized: {enhanced_context_data['cache_optimized']}, Source: {enhanced_context_data.get('context_source', 'unknown')})")
                print(f"🎯 Context scope (streaming): {enhanced_context_data.get('document_count', 0)} documents from {enhanced_context_data.get('chunk_count', 0)} retrieved chunks")
            except Exception as e:
                print(f"⚠️  Warning: Could not get chunk-based enhanced context for streaming: {e}")
                # Fallback to old method if needed (but log the fallback)
                try:
                    enhanced_context_data = self.context_enhancer.get_enhanced_context(query)
                    print(f"📋 Fallback context (streaming): {enhanced_context_data['context_length']} chars (ALL documents - not optimized)")
                except Exception as fallback_error:
                    print(f"⚠️  Warning: Fallback context for streaming also failed: {fallback_error}")
